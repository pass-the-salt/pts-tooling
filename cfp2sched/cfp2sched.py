...
for talk_details in data:
    ...
    try:
        fn = talk_details['options']['picture-upload']['content_file']
        ...
        if os.path.exists(os.path.join(INDIR, potential_image_file)):
            ...
            # we build there the filename of the square crop version of the uploaded image.
            # the filename will fill the YAML "image" header which is used to build the correspondant twitter card image header.
            # note: the crop of the image has to be done by hand for the moment.
            potential_crop_image_file = 'hotcrp-paper{}-picture-upload-crop{}'.format(talk_details['pid'], file_name_extension)
            imageheader = '/{}/{}'.format(OUTIMGS,potential_crop_image_file)
